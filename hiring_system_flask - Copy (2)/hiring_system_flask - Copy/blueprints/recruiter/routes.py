...
from sqlalchemy import or_, func, inspect, text            # ⇐ func اضافه شد
...
def ensure_branch_manager_mobile_column():
    """Add branch_manager_mobile column if missing (legacy DBs)."""
    insp = inspect(db.engine)
    cols = [c["name"] for c in insp.get_columns("hiring_case")]
    if "branch_manager_mobile" not in cols:
        with db.engine.connect() as conn:
            conn.execute(text("ALTER TABLE hiring_case ADD COLUMN branch_manager_mobile VARCHAR(20)"))

# ------------------------ داشبورد + جست‌وجو ------------------------
@recruiter_bp.get("/dashboard")
@login_required
def dashboard():
    ensure_branch_manager_mobile_column()
